...
# 06_case_studies.R
#
# Purpose: Analyze six historical case studies to show patterns that may help identify breakouts and understand how the models work. 
#
# This script:
#     1. Analyzes and creates visualizations for Wyatt Johnston
#     2. Analyzes and creates visualizations for Pavel Zacha
#     3. Analyzes and creates visualizations for Zach Benson
#
# Inputs:
...
#
# Outputs:
#   Visualizations on Wyatt Johnston's career and why the model liked him
#   Visualizations on Pavel Zacha's career and why the model liked him
#   Visualizations on Zach Benson's career and why the model liked him
#
##########################################################
...
# Wyatt Johnston's Career Trajectory Plot
johnston_traj <- ggplot(johnston, aes(x = season)) +
  geom_line(
    aes(y = I_F_points,
        color = "Points"),
    linewidth = 1.5
...
ggsave(
  here("figures", "johnstontrajectory.png"),
  johnston_traj,
  width = 10,
  height = 5,
  dpi = 300
...
# Wyatt Johnston compared to the NHL plot
johnston_comp <- ggplot(scatter_data,
       aes(x = value,
           y = I_F_points)) +
  geom_point(alpha = .15,
             color = "gray50") +
...
# Wyatt Johnston compared to the league density plots (only the HDxG plot was used in the report)
johnston_xg <- ggplot(test, aes(I_F_xGoals)) +
...
# Corsi%
johnston_corsi <- ggplot(test, aes(onIce_corsiPercentage)) +
  geom_density(fill = "steelblue") +
  geom_vline(
    xintercept = johnston |>
      filter(season == 2022) |>
...
ggsave(
  here("figures", "johnstoncorsi.png"),
  johnston_corsi,
  width = 10,
  height = 5,
  dpi = 300
...
# HDxG (used in the report)
johnston_hdxg <- ggplot(test, aes(I_F_highDangerxGoals)) +
  geom_density(fill = "steelblue") +
  geom_vline(
    xintercept = johnston |>
      filter(season == 2022) |>
...
##########################################################
# Case Study Two: Pavel Zacha 
##########################################################

# Prepare Zacha's data
zacha <- model_data |>
...
# Combining Zacha's data with his current season because it is useful in these visualizations
...
# First plot: Zacha's career trajectory
zacha_traj <- ggplot(zacha, aes(x = season)) +
  geom_line(
    aes(y = I_F_points,
        color = "Points"),
    linewidth = 1.5
...
ggsave(
  here("figures", "zachtrajectory.png"),
  zacha_traj,
  width = 10,
  height = 5,
  dpi = 300
...
# Pavel Zacha compared to the league; scatterplot
zacha_comp <- ggplot(scatter_data,
       aes(x = value,
           y = I_F_points)) +
  geom_point(alpha = .15,
             color = "gray50") +
...
# Creating a placeholder in order to calculate percentiles - full integration of the historical set and the current set comes in 08_predictions.R
...
# Pavel Zacha's percentile evolution plot
zachapct_plot <- 
  ggplot(zacha_pct,
       aes(x = season,
           y = percentile,
...
##########################################################
# Case Study Three: Zach Benson
##########################################################

# Preparing Zach Benson's data
benson <- test |> 
...
# Zach Benson's career trajectory plot
benson_traj <- ggplot(benson, aes(x = season)) +
  geom_line(
    aes(y = I_F_points,
        color = "Points"),
    linewidth = 1.5
...
# Zach Benson's percentile evolution plot
bensonpct_plot <- ggplot(benson_pct,
       aes(x = season,
           y = percentile,
           color = metric)) +
  geom_line(linewidth = 1.2) +
...
# Preparing Data for Benson's final visualization: the gap between his xg percentile and points percentile
...
benson_gap_plot <- ggplot(benson_gap,
       aes(x = season,
           y = gap)) +
  geom_line(
    linewidth = 1.5,
